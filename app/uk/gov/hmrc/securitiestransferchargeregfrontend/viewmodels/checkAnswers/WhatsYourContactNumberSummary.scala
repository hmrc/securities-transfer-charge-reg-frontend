...
object WhatsYourContactNumberSummary  {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(WhatsYourContactNumberPage).map {
      answer =>

        SummaryListRowViewModel(
          key     = "whatsYourContactNumber.checkYourAnswersLabel",
          value   = ValueViewModel(HtmlFormat.escape(answer).toString),
          actions = Seq(
            ActionItemViewModel("site.change", routes.WhatsYourContactNumberController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("whatsYourContactNumber.change.hidden"))
          )
        )
    }
}
