...
object WhatsYourEmailAddressSummary  {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(WhatsYourEmailAddressPage).map {
      answer =>

        SummaryListRowViewModel(
          key     = "whatsYourEmailAddress.checkYourAnswersLabel",
          value   = ValueViewModel(HtmlFormat.escape(answer).toString),
          actions = Seq(
            ActionItemViewModel("site.change", routes.WhatsYourEmailAddressController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("whatsYourEmailAddress.change.hidden"))
          )
        )
    }
}
