...
trait SubscriptionConnector:
  def subscribeAndEnrolIndividual(userId: String)(nino: String)(userAnswers: UserAnswers): Future[Unit]
...
class SubscriptionConnectorImpl @Inject() ( registrationClient: RegistrationClient,
  ...
  override def subscribeAndEnrolIndividual(userId: String)(nino: String)(userAnswers: UserAnswers): Future[Unit] = {
    for {
  ...
  private def enrol(subscriptionId: String, nino: String): Future[Unit] = {
    registrationClient.enrolIndividual(IndividualEnrolmentDetails(subscriptionId, nino)).flatMap {
      case Right(EnrolmentSuccessful) => Future.successful(())
      case Right(_) =>
