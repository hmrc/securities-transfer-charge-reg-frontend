...
import uk.gov.hmrc.securitiestransferchargeregfrontend.controllers.organisations.routes.ContactEmailAddressController
...
class ContactEmailAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ContactEmailAddress Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("test@email.com"), NormalMode)(request, messages(application)).toString
      }
    }
